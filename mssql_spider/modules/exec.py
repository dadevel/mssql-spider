...
def job(client: MSSQLClient, command: str, language: str = 'powershell') -> dict[str, Any]:
    ...
    try:
        ...
        assert "'" not in command
        assert language in subsystems
        job_name, step_name = random_string(), random_string()
        client.query_database(
            'msdb',
            'BEGIN TRY '
            f"EXEC dbo.sp_add_job '{job_name}' "
            f"EXEC dbo.sp_add_jobstep @job_name='{job_name}',@step_name='{step_name}',@subsystem='{subsystems[language]}',@command='{command}' "
